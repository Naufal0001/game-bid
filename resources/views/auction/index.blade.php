            ...
            <div class="flex flex-col md:flex-row gap-8">
                ...
                <aside class="w-full md:w-1/4">
                    <form action="{{ route('auctions.index') }}" method="GET"
                        class="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] sticky top-24">
                        ...
                        <div class="mb-6">
                            <h4 class="font-bold text-lg mb-3 bg-yellow-400 inline-block px-2 border-2 border-black">GAME
                            </h4>
                            <div class="space-y-2">
                                @foreach (['CS2', 'Tower of Fantasy', 'Valorant', 'Ragnarok'] as $gameName)
                                    <label class="flex items-center space-x-3 cursor-pointer group">
                                        ...
                                        <span
                                            class="font-bold text-gray-700 group-hover:text-black">{{ $gameName }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            <h4 class="font-bold text-lg mb-3 bg-teal-400 inline-block px-2 border-2 border-black">TIPE ITEM
                            </h4>
                            <div class="space-y-2">
                                @foreach (['Account', 'Misc', 'Currency', 'Mount', 'Cosmetic', 'Equipment'] as $catName)
                                    <label class="flex items-center space-x-3 cursor-pointer group">
                                        ...
                                        <span
                                            class="font-bold text-gray-700 group-hover:text-black">{{ $catName }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            <h4 class="font-bold text-lg mb-3 bg-pink-400 inline-block px-2 border-2 border-black">HARGA
                                (IDR)</h4>
                            <div class="flex flex-col gap-3">
                                <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min"
                                    class="w-full border-2 border-black p-2 font-bold focus:outline-none focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
                        ...
                        <button type="submit"
                            class="w-full py-3 bg-black text-white font-black uppercase tracking-widest border-2 border-transparent hover:bg-white hover:text-black hover:border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
                            TERAPKAN
                        </button>

                        @if (request()->hasAny(['game', 'category', 'min_price', 'max_price', 'search']))
                            <a href="{{ route('auctions.index') }}"
                                class="block text-center mt-3 text-xs font-bold underline text-red-600">Reset Filter</a>
                        @endif
                    </form>
                </aside>
                ...
                <div class="w-full md:w-3/4">
                    ...
                    <form action="{{ route('auctions.index') }}" method="GET" class="mb-8">
                        ...
                        <div class="flex flex-col md:flex-row gap-4">
                            ...
                            <button type="submit"
                                class="px-8 py-4 bg-black text-white font-black uppercase tracking-widest border-4 border-transparent hover:bg-yellow-400 hover:text-black hover:border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all flex items-center justify-center gap-2">
                                CARI
                            </button>
                        </div>
                    </form>
                    ...
                    @if ($auctions->count() > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach ($auctions as $auction)
                                <div
                                    class="group bg-white border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 flex flex-col h-full relative">

                                    <div
                                        class="absolute top-0 left-0 bg-yellow-400 border-b-2 border-r-2 border-black px-3 py-1 z-10">
                                    ...
                                    <div class="h-48 w-full border-b-4 border-black bg-gray-100 overflow-hidden relative">
                                        ...
                                        <div
                                            class="absolute bottom-0 right-0 bg-black text-white text-xs font-bold px-2 py-1">
                                    ...
                                    <a href="{{ route('auctions.show', $auction->id) }}"
                                        class="block w-full py-3 bg-black text-white text-center font-black uppercase tracking-widest hover:bg-yellow-400 hover:text-black border-t-4 border-black transition-colors">
                                        LIHAT DETAIL
                                    </a>
                    ...
                    @else
                        <div class="bg-white border-4 border-black p-10 text-center shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                            ...
                            <p class="font-medium text-gray-600">Barang dengan kata kunci atau filter tersebut tidak
                                ditemukan.</p>
                            <a href="{{ route('auctions.index') }}"
                                class="inline-block mt-4 px-6 py-2 bg-yellow-400 border-2 border-black font-bold shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none">Reset
                                Pencarian</a>
                        </div>
                    @endif
                </div>
